...
func TestLimit_Fill_MultipleOrders_FIFO(t *testing.T) {
	...
	// First Match With ask1 (FIFO)
	assertFloat(t, 1.0, matches[0].SizeFilled, "First match size")
	assertEqual(t, "1", matches[0].Ask.UserID, "First match should be with Userid:1 (FIFO)")

	// First Match With ask2
	assertFloat(t, 0.5, matches[1].SizeFilled, "Second match size")
	assertEqual(t, "2", matches[1].Ask.UserID, "Second match should be with Userid:2")
...
func TestLimit_Fill_SelfTradePrevention(t *testing.T) {
	...
	// Should have no matches (self-trade prevention)
...
func TestOrderbook_PlaceLimitOrder_MultipleMatches(t *testing.T) {
	...
	// Second Match with best price
	assertFloat(t, 50100.0, matches[1].Price, "Second match price")
...
func TestOrderbook_PlaceLimitOrder_PriceTimePriority(t *testing.T) {
	...
	assertEqual(t, 1, len(matches), "Should have 1 match")
	assertEqual(t, "1", matches[0].Ask.UserID, "Should match with 1 first (FIFO)")

	assertEqual(t, 1, len(ob.Asks()), "Should have 1 ask remaining")
	assertEqual(t, "2", ob.Asks()[0].Orders[0].UserID, "Remaining order should be 2")
}
